...
public sealed class CreateHabitDtoValidator : AbstractValidator<CreateHabitDto>
{
    ...
    public CreateHabitDtoValidator()
    {
        ...
        RuleFor(x => x.Target.Value)
            .GreaterThan(0)
            .WithMessage("Target value must be greater then 0");
        ...
        When(x => x.Milestone is not null, () =>
        {
            RuleFor(x => x.Milestone!.Target)
                .GreaterThan(0)
                .WithMessage("<Milestone target must be greater then 0");
        });
    ...
    public static bool IsTargetUnitCompatibleWithType(HabitType type, string unit)
    {
        ...
        return type switch
        {
            ...
            //Measurable habits can use any of the allowed uinits 
